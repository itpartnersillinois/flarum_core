  ...
  attrs() {
    return {
      className: classList([
        'DiscussionListItem',
        this.active() ? 'active' : '',
        //this.props.user.isHidden() ? 'DiscussionListItem--hidden' : ''
      ])
    };
  }
   ...
   // const userId = this.props.params.userId;
    ...
    return (
      <div {...attrs}>
        ...
        <a className={'Slidable-underneath Slidable-underneath--left Slidable-underneath--elastic'}
        ...
        <div className={'DiscussionListItem-content Slidable-content read'}>
          <a href={user ? app.route.user(user) : '#'}
            ...
            title={extractText(app.translator.trans('core.forum.discussion_list.started_text', {user: user}))}
          ...
          <a
            ...
            <h3 className="DiscussionListItem-title">{highlight(user.username(), this.highlightRegExp)}</h3>
            { <ul className="DiscussionListItem-info">{user.email()/*listItems(this.infoItems().toArray())*/}</ul> }
          </a>
